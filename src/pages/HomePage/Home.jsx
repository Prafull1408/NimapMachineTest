...
function Home() {
  ...
  async function fetchMovies(getKey) {
    try {
      ...
      const response = await fetch(
        `https://api.themoviedb.org/3/movie/popular?api_key=${getKey}&language=en-US&page=${pageCount}`
      );
  ...
  return (
    <div className="w-full min-h-screen overflow-x-hidden overflow-y-auto bg-prussian-blue p-4 lg:px-8">
      <div className="font-mono text-blue-green pb-4">Page {pageCount} of {pages}</div>
